...
struct altera_c5_fpll {
	...
	struct device	*dev;
	unsigned long	rate;
	unsigned char	fractional_carry_bit;
	struct clk	*ref_clk;
				   ...
				   unsigned long parent_rate)
{
	...
	writel(0x00u, fpll->base + C5_FPLL_START_REG_ADDR);

	ret = readl_poll_timeout(fpll->base + C5_FPLL_STATUS_REG_ADDR,
...
static int altera_c5_fpll_parse_dt(struct altera_c5_fpll *fpll)
{
	...
	ret = of_property_read_u32(np, "adi,fractional-carry-bit", &tmp);
	if (ret < 0)
		return ret;

	fpll->fractional_carry_bit = tmp;
